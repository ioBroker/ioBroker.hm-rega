...
rega.prototype = {
    ...
    script: function (script, callback) {
        ...
        if (this.pendingRequests > 0) {
            ...
            if (!(this.counter % 100)) this.logger.warn('Pending request for more than ' + (this.counter * 250) + ' ms');

            setTimeout(function (_script, _callback) {
                that.script(_script, _callback);
            }, 250, script, callback);
            return;
        }
        this.counter = 0;
        ...
        var post_req = http.request(post_options, function (res) {
            ...
            res.on('end', function () {
                if (that.pendingRequests > 0) that.pendingRequests -= 1;

...
const Rega = function (options) {
    ...
    this.loadStringTable = function (language, callback) {
        ...
        that.loadTranslation(language, (translation) => {
            request.get({
                ...
                const data = body;
                const str = iconv.decode(data, 'ISO-8859-1');
    ...
    this.script = function (script, callback) {
        ...
        this.counter = 0;
        this.logger.debug('--> ' + this.pendingRequests[0].script.toString().slice(0, 80).replace(/\n/g, ' '));
        ...
        const post_req = this.protocolModule.request(post_options, function (res) {
            let data = '';

...
const Rega = function (options) {
    ...
    this.init = function () {
        if (this.options.ccuIp) {
            request('http://' + this.options.ccuIp + '/ise/checkrega.cgi', (error, response, body) => {
    ...
    this.loadTranslation = function (lang, callback) {
        ...
        request.get({url: 'http://' + that.options.ccuIp + '/webui/js/lang/' + lang + '/translate.lang.js', encoding: null}, (err, res, body) => {
            if (res.statusCode === 200) {
                try {
                    ...
                    request.get({url: 'http://' + that.options.ccuIp + '/webui/js/lang/' + lang + '/translate.lang.stringtable.js', encoding: null}, (err, res, body) => {
                        if (res.statusCode === 200) {
                            const str = unescape(iconv.decode(body, 'ISO-8859-1'));
                            const jscode = str.replace(/jQuery\./g, "");
                        ...
                        request.get({url: 'http://' + that.options.ccuIp + '/webui/js/lang/' + lang + '/translate.lang.extensionV.js', encoding: null}, (err, res, body) => {
                            if (res.statusCode === 200) {
                                const str = unescape(iconv.decode(body, 'ISO-8859-1'));
                                const jscode = str.replace(/jQuery\./g, '');
    ...
    this.loadStringTable = function (language, callback) {
        ...
        that.loadTranslation(language, translation => {
            request.get({url: 'http://' + that.options.ccuIp + '/config/stringtable_de.txt', encoding: null}, (err, res, body) => {

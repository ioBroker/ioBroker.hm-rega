    ...
    script(script, callback) {
        ...
        if (script) {
            ...
            if (this.pendingRequests.length > 1) {
                ...
                if (this.counter >= 100) {
                    // clear queue if a script stucks in there for too long
                    this.pendingRequests = [];
                    this.logger.warn('Pending queue cleared because got blocked for too long - all pending requests dropped');
                } // endIf
                return;
            }
